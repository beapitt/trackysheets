...
export default function EditCategory() {
  ...
  const inputClass = "w-full p-2 border border-gray-300 rounded focus:ring-1 focus:ring-green-600 outline-none";
    ...
    <div className="flex min-h-screen bg-gray-100 text-left font-sans">
      <aside className="w-48 bg-[#1a3a1a] text-white p-6 shrink-0 font-sans">
        ...
        <nav className="space-y-2 text-sm">
          <Link to="/admin/templates" className="block px-4 py-2 rounded hover:bg-white/10 no-underline text-white font-bold tracking-tight">Templates</Link>
          <Link to="/admin/categories" className="block px-4 py-2 rounded bg-white/10 border-l-4 border-green-400 no-underline text-white font-bold tracking-tight">Categories</Link>
          <Link to="/admin/settings" className="block px-4 py-2 rounded hover:bg-white/10 no-underline text-white font-bold tracking-tight">Settings</Link>
        </nav>
      </aside>

      <main className="flex-1 p-8 text-left">
        <div className="max-w-2xl bg-white rounded-lg shadow-sm p-8 border border-gray-200">
          <h1 className="text-xl font-bold uppercase text-[#2D5A27] border-b pb-4 mb-6">
            {id && id !== 'new' ? 'Edit Category' : 'Add New Category'}
          </h1>
          
          {message && <div className={`p-4 mb-6 rounded font-bold text-xs ${message.includes('✅') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>{message}</div>}
