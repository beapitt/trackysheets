  ...
  useEffect(() => {
    async function fetchCategoryData() {
      ...
      if (catData) {
        ...
        const { data: tData } = await supabase.from('templates').select('*').eq('category_id', catData.id).order('created_at', { ascending: false })
        ...
        <div className="flex flex-row items-start gap-12">
          ...
          <main className="flex-1 min-w-0">
            ...
            {templates.length > 0 ? (
              ...
              <p className="text-gray-500 italic">No templates found in this category.</p>
            )}
          </main>
            ...
            <div>
               ...
               <div className="flex flex-col gap-0.5">
                  {filteredCategories.slice(0, 8).map((cat) => (
                    <Link key={cat.id} to={`/category/${cat.slug}`} className={`text-[12px] font-bold no-underline py-2 border-b border-gray-50 hover:bg-gray-50 px-2 rounded-md transition-all ${cat.slug === slug ? 'text-[#1F5C3E] bg-gray-50' : 'text-gray-500'}`}>
                      {cat.name}
                    </Link>
                  ))}
               </div>
