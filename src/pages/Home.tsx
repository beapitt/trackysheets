            ...
            <section className="pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={s.n} className="bg-[#EAF3DE] rounded-xl p-4">
                    <span className="text-[11px] font-bold text-transparent block mb-1" 
                          style={{ WebkitTextStroke: "1.2px #3B6D11" }}>{s.n}</span>
                    <span className="text-[22px] font-bold text-[#27500A] block leading-none mb-1">{s.num}</span>
                    <span className="text-[10px] text-[#3B6D11] uppercase font-bold tracking-wide">{s.sub}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <h2 className="text-[15px] font-bold text-[#1f2937]">Newly released</h2>
                  <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-[#EAF3DE] text-[#1F5C3E] uppercase">NEW</span>
                </div>
                <Link to="/templates" className="text-[12px] font-bold text-[#1F5C3E] no-underline flex items-center gap-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                {templates.slice(0, 6).map((template) => (
                  <Link key={template.id} to={`/template/${template.slug}`} className="group no-underline block">
                    ...
                    <h3 className="text-[14px] font-bold text-gray-900 leading-tight mb-1 line-clamp-1">{template.title}</h3>
                    <div className="flex justify-between items-center text-[10px] font-bold">
                      <span className="text-gray-400 uppercase tracking-wider">{template.software || 'Google Sheets'}</span>
                      <span className="text-[#1F5C3E]">FREE →</span>
                    </div>
                  </Link>
                ))}
              </div>
