...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-[100] w-full shadow-md font-inter" style={{ backgroundColor: '#1F5C3E' }}>
      ...
      {/* BARRA SUPERIORE (LOGO E RICERCA) */}
      ...
      {/* BARRA INFERIORE (CATEGORIE + ALL TEMPLATES) */}
      <nav className="hidden md:flex h-[42px] items-center bg-black/10 border-t border-white/5">
        <div className="w-full max-w-[1440px] mx-auto px-10 flex items-center justify-between">
          ...
          {/* ALL TEMPLATES (Posizionato sotto Help) */}
          <Link 
            to="/templates" 
            className={`text-[11px] font-black uppercase tracking-[0.15em] no-underline transition-all pb-1 border-b-2 ${
              location.pathname === '/templates' ? 'text-white border-white' : 'text-white/60 border-transparent hover:text-white'
            }`}
          >
            All Templates
          </Link>
      ...
      {/* MENU MOBILE SCOMPARSA */}
      {isMenuOpen && (
        <div className="absolute top-[64px] left-0 w-full bg-white border-b border-gray-200 shadow-2xl animate-in slide-in-from-top-2 duration-200 md:hidden text-left">
          <div className="px-6 py-8">
            ...
            <div className="grid grid-cols-1 gap-y-1">
              <Link to="/templates" onClick={() => setIsMenuOpen(false)} className="text-[15px] font-bold py-3 border-b border-gray-50 no-underline text-[#1F5C3E]">All Templates</Link>
              {categories.map((cat) => (
                <Link
                  key={cat.id}
